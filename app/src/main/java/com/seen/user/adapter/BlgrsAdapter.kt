...
import com.seen.user.model.BloggersItem
import com.seen.user.utils.SharedPreferenceUtility
import kotlinx.android.synthetic.main.item_product_wise_search_layout.view.*

class BlgrsAdapter(
    private val context: Context,
    private val bloggersList: ArrayList<BloggersItem?>?,
    private val findNavController: NavController
) : RecyclerView.Adapter<BlgrsAdapter.BlgrsAdapterVH>() {
    inner class BlgrsAdapterVH(itemView : View) : RecyclerView.ViewHolder(itemView)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BlgrsAdapterVH {
        val view= LayoutInflater.from(context).inflate(R.layout.item_product_wise_search_layout, parent, false)
        return BlgrsAdapterVH(view)
    }

    override fun onBindViewHolder(holder: BlgrsAdapterVH, position: Int) {
        val brandsItem = bloggersList?.get(position)
        if (brandsItem!=null){
